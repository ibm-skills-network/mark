...
describe('DatabaseConnectionMiddleware', () => {
  ...
  beforeEach(() => {
    ...
    middleware = new DatabaseConnectionMiddleware(prismaService as any, circuitBreaker as any);
  });

  it('skips database check for health endpoints', async () => {
    const next = jest.fn();

    await middleware.use(createRequest('/health/liveness'), createResponse(), next);
  ...
  it('executes a lightweight query when route is not a health check', async () => {
    ...
    await middleware.use(createRequest('/api/resource'), createResponse(), next);
  ...
  it('adds degraded header when circuit breaker raises an error', async () => {
    ...
    expect(response.setHeader).toHaveBeenCalledWith('X-Database-Status', 'degraded');
    expect(next).toHaveBeenCalledTimes(1);
  });
});
