 ...
 */
export const DataTransform = (options: TransformOptions = {}): MethodDecorator =>
  Reflect.metadata(TRANSFORM_METADATA_KEY, options);
...
export class DataTransformInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    return next.handle().pipe(
      map((data: unknown) => this.transformResponse(data, options))
    );
  }
   ...
   */
  private getTransformOptions(context: ExecutionContext): TransformOptions | null {
   ...
   */
  private transformRequest(context: ExecutionContext, options: TransformOptions): void {
    if (!options.decodeRequest) return;

    const request = context.switchToHttp().getRequest() as Record<string, unknown>;

    if (request.body && typeof request.body === 'object') {
      request.body = this.transformData(request.body, options, 'decode');
    }

    if (request.query && typeof request.query === 'object') {
      request.query = this.transformData(request.query, options, 'decode');
    }
  }
   ...
   */
  private transformData(data: unknown, options: TransformOptions, operation: 'encode' | 'decode'): unknown {
    if (!data || typeof data !== 'object') return data;

    if (Array.isArray(data)) {
      return data.map((item: unknown) => this.transformData(item, options, operation));
    }
   ...
   */
  private shouldTransformField(key: string, value: unknown, fields?: string[]): boolean {
