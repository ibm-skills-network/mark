...
test('author settings persist and reflect in preview', async ({ page }) => {
  ...
  await page.getByRole('switch', { name: 'Enforce a strict time limit' }).click();
  ...
  // Set retry behavior
  await page.getByRole('button', { name: 'How many attempts do learners' }).click();
  await page.getByText('Never wait to retry').click();

  // Set question display options
  await page.getByRole('button', { name: 'All questions in one page All' }).click();
  await page.getByRole('button', { name: 'Strict Order Questions always' }).click();
